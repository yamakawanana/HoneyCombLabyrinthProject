...
Level* LevelChanger::CreateLevelProcessing(TaskId task_id) {
  Level* level = nullptr;
  ...
  case TaskId::kRuleLevelTask:
    level = new RuleLevel(task_id, task_manager_);
...
bool LevelChanger::PhaseUpdate(float time) {
  //レベルのフェーズが終了済みフェーズの場合、レベルチェンジャーの終了フェーズに遷移する
   if (level_ != nullptr && level_->CheckIsEnded()) {
    phase_type_ = kEnding;
  ...
  else if (task_id_ == kRuleLevelTask && level_transition_ == kEnd) {
    level_transition_ = kCreateNextLevel;
    DEBUG_PRINT(kRuleLevelTask 次のレベルの生成);
  }
  ...
  case LevelTransitionType::kCreateNextLevel: {
    ...
    //次のレベルを生成する
    level_ = CreateLevelProcessing(task_id_);
...
bool LevelChanger::Finalize(float time) {
  ...
  if (task_id_ == kNone) {
    ...
    task_id_ = kRuleLevelTask;
    DEBUG_PRINT(タスクIDを更新した);
  }
  else if (task_id_ == kRuleLevelTask) {
